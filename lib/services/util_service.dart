...
class UtilService extends GetxService {
  ...
  Future<UtilService> init() async {
    await 1.delay();
    return this;
  }

  showToaster(String title, String message) {
    Get.snackbar(title, message);
  }

  showLoader({msg: "Please Wait.."}) async {
    this.isLoading = true;
  ...
  showAlert(
    ...
    bool backgroundDismissal = true,
  }) async {
    dynamic dialogRes = await Get.defaultDialog(
  ...
  final List<Map<String, String>> countriesJSON = [
    ...
    {"name": "Pakistan", "dial_code": "0", "code": "PK"},
  ...
  Future<void> uploadFile() async {
    // Fetch the application directory
...
//  files.sort((a, b) => b.modificationDateSync().compareTo(a.modificationDateSync()));

    // Take the first file in the sorted list, which should be the most recent one
    final mostRecentFile = files.first;

    // Convert the file into a list of bytes
    final fileBytes = await mostRecentFile;
...
//  );
  ...
  avatarFromVideo() async {
    try {
      var localAvatar = await _picker.pickVideo(
      ...
      if (localAvatar != null) {
        ...
        var video = File(localAvatar.path);
        final appDir = await syspaths.getApplicationDocumentsDirectory();
        final fileName = path.basename(localAvatar.path);
        video = await video.copy('${appDir.path}/$fileName');
        var uploadRes;
        //  await uploadVideo(video, fileName, avatarPath!);
        hideLoader();
        return uploadRes;
      }
      hideLoader();
      ...
      hideLoader();
      Get.snackbar("Error", "An Error occured will uploading video");
      return null;
    }
  }
  ...
  avatarFromCamera() async {
    try {
      ...
      if (localAvatar != null) {
        avatarPath = localAvatar.path;
        var image = File(localAvatar.path);
        final appDir = await syspaths.getApplicationDocumentsDirectory();
        final fileName = path.basename(localAvatar.path);
        image = await image.copy('${appDir.path}/$fileName');
        var uploadRes;
        // =
        //  await uploadPicture(image, fileName, localAvatar.path);
        hideLoader();
        return uploadRes;
      }
      hideLoader();
      ...
      hideLoader();
      Get.snackbar("Error", "An Error occured will uploading image");
      return null;
    }
  }
      ...
      actions: [
        Container(
          ...
          decoration: BoxDecoration(
            border: Border(
              top:
                  BorderSide(color: Color.fromRGBO(242, 243, 246, 1), width: 1),
            ),
          ),
          child: !isSingleButton
              ? Row(
  ...
  // settingModalBottomSheet(context, {bool ShowVideo = false}) {
  //   return showModalBottomSheet(
  //     context: context,
  //     builder: (BuildContext bc) {
  //       return Container(
  //         height: ShowVideo == false ? Get.height * .22 : Get.height * .38,
  ...
  //             InkWell(
  //               onTap: () async {
  //                 var res = await avatarFromCamera();
  //                 Navigator.pop(context, res);
  //               },
  //               child: Container(
  //                 padding: EdgeInsets.all(10),
  //                 margin: EdgeInsets.only(left: 6, right: 6, bottom: 6, top: 6),
  //                 decoration: BoxDecoration(
  ...
  //             InkWell(
  //               onTap: () async {
  //                 var res = await avatarFromGallery();
  //                 Navigator.pop(context, res);
  //               },
  //               child: Container(
  //                 padding: EdgeInsets.all(10),
  ...
  //                 : InkWell(
  //                     onTap: () async {
  //                       var res = await avatarVideoFromGallery(null);
  //                       Navigator.pop(context, res);
  //                     },
  //                     child: Container(
  //                       padding: EdgeInsets.all(10),
  ...
  // Future<bool> isPortraitImage(String imageUrl) async {
  //   ImageProperties? imageProperties;

  //   try {
  //     imageProperties = await FlutterNativeImage.getImageProperties(imageUrl);
  //   } catch (e) {
  //     print('Error getting image properties: $e');
  //   }
